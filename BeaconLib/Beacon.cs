...
namespace BeaconLib
{
    ...
    public class Beacon : IDisposable
    {
        ...
        {
            UnityEngine.Debug.Log("ProbeReceived");
        ...
        internal static bool HasPrefix<T>(IEnumerable<T> haystack, IEnumerable<T> prefix)
        {
            ...
            bool haystackEqualsPrefix = true;
            int len = Math.Min(haystack.Count(), prefix.Count());
            for(int i=0;i<len;i++){
                if( !haystack.ElementAt(i).Equals(prefix.ElementAt(i)) ){
                    haystackEqualsPrefix = false;
                }
            }
            return haystack.Count() >= prefix.Count() && haystackEqualsPrefix;
        }
